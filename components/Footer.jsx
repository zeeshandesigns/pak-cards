...
const Footer = () => {
  ...
  const linkSections = [
    ...
    },
    {
      title: "CONTACT",
      links: [
        { text: "+92-300-1234567", path: "tel:+923001234567", icon: PhoneIcon },
        {
          text: "support@pakcards.com",
          path: "mailto:support@pakcards.com",
          icon: MailIcon,
        },
        { text: "Karachi, Pakistan", path: "/", icon: MapPinIcon },
      ],
    },
  ];
  ...
  return (
    <footer className="mx-4 sm:mx-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 sm:gap-10 py-8 sm:py-10 border-b border-slate-500/30 text-slate-500">
          <div className="max-w-md">
            <Link
              href="/"
              className="text-3xl sm:text-4xl font-semibold text-slate-700"
            >
            ...
            <p className="max-w-[410px] mt-4 sm:mt-6 text-xs sm:text-sm leading-relaxed">
              Your trusted marketplace for instant digital gift cards. Buy and
              sell gift cards securely with fast delivery and best prices in
              Pakistan.
            </p>
            <div className="flex items-center gap-3 mt-4 sm:mt-5">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 bg-slate-100 hover:scale-105 hover:bg-green-50 hover:border border-green-300 transition rounded-full"
                >
                  <item.icon />
                </Link>
              ))}
            ...
            <div className="mt-6 pt-6 border-t border-slate-200">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div className="px-3 py-2 bg-slate-100 rounded text-xs font-medium text-slate-600">
                  💳 Bank Transfer
                </div>
                <div className="px-3 py-2 bg-slate-100 rounded text-xs font-medium text-slate-600">
                  🔒 SSL Secure
          ...
          <div className="flex flex-wrap justify-between w-full lg:w-auto lg:flex-1 lg:max-w-2xl gap-8 sm:gap-10 text-sm">
            {linkSections.map((section, index) => (
              <div key={index} className="min-w-[140px]">
                <h3 className="font-semibold text-slate-700 mb-3 sm:mb-5">
                  {section.title}
                </h3>
                <ul className="space-y-2 sm:space-y-2.5">
                  {section.links.map((link, i) => (
                    <li key={i} className="flex items-center gap-2">
                      ...
                      <Link
                        href={link.path}
                        className="hover:text-green-600 hover:underline transition text-xs sm:text-sm"
                      >
                        {link.text}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between py-4 sm:py-5 gap-3 text-xs sm:text-sm text-slate-500">
          <p>Copyright 2025 © PakCards. All Rights Reserved.</p>
